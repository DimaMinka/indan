...
get_header(); ?>
	...
	<?php } else { ?>
		<?php $blog_heading = ot_get_option( 'blog_heading' ); ?>
		<?php if ( empty( $blog_heading ) ) { ?>
			<?php $blog_heading = 'enable' ?>
		<?php } ?>
			...
			<?php if ( $blog_heading == 'enable' ) { ?>
				<div class="entry-header-wrapper" <?php if ( $background_overlay_color_for_heading != '#ffffff' || $background_overlay_opacity_for_heading != '.1' ) { ?>style="<?php if ( $background_overlay_color_for_heading != '#ffffff' ) { ?>background: <?php echo esc_attr( $background_overlay_color_for_heading ); ?>;<?php } ?> <?php if ( $background_overlay_opacity_for_heading != '.1' ) { ?>background: <?php echo esc_attr( $rgba ); ?>;<?php } ?>"<?php } ?>>
					<header class="entry-header clearfix">
						<?php //$title = single_post_title(); ?>
						<h1 class="entry-title" <?php if ( $color_for_heading != '#444444' ) { ?>style="color: <?php echo esc_attr( $color_for_heading ); ?>";<?php } ?>><?php single_post_title(); ?></h1>
					</header><!-- .entry-header -->
				</div>
			<?php } ?>
		<?php } ?>
	...
	<div id="main" class="clearfix <?php echo 'blog-heading-'.esc_attr( $blog_heading ); ?>">
		<div id="primary">
