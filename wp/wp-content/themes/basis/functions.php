...
function mega_setup() {
	...
	/* Make Basis available for translation.
				...
				<div class="product-list-cart">
					...
					<?php foreach($woocommerce->cart->cart_contents as $cart_item): //var_dump($cart_item); ?>
						...
						<div class="woocommerce-cart-checkout">
							...
							<a class="button alt" href="<?php echo esc_url( get_permalink( get_option( 'woocommerce_checkout_page_id' ) ) ); ?>"><?php _e( 'Checkout', 'mega' ); ?></a>
						</div>
					</ul>
				</div>
			<?php } ?>
	...
	function mega_woocommerce_template_loop_product_thumbnail() {
		...
		echo '<span class="'. sanitize_html_class( $classes ) .'">';
...
function mega_register_required_plugins() {
	$plugins = array(
		...
		array(
			...
			'source'   				=> get_template_directory() . '/inc/plugins/showbiz.zip', // The plugin source
			'required' 				=> false, // If false, the plugin is only 'recommended' instead of required
			'version' 				=> '', // E.g. 1.0.0. If set, the active plugin must be this version or mega, otherwise a notice is presented
			'force_activation' 		=> false, // If true, plugin is activated upon theme activation and cannot be deactivated until theme switch
			'force_deactivation' 	=> true, // If true, plugin is deactivated upon theme switch, useful for theme-specific plugins
...
function mega_content_nav( $nav_id ) {
	...
	if ( $wp_query->max_num_pages > 1 ) : ?>
		<nav id="<?php echo sanitize_html_class( $nav_id ); ?>">
			...
			<div class="nav-previous"><?php next_posts_link( __( 'Older Posts <i class="nav-pagination-single-right"></i>', 'mega' ) ); ?></div>
			<div class="nav-next"><?php previous_posts_link( __( '<i class="nav-pagination-single-left"></i> Newer Posts', 'mega' ) ); ?></div>
		</nav><!-- #nav-above -->
	<?php endif;
}
 ...
 */
 function mega_enqueue_scripts() {
	...
	wp_register_script( 'background-check.min', get_template_directory_uri() . '/js/background-check.min.js', array(), false, false );
	wp_register_script( 'pace.min', get_template_directory_uri() . '/js/pace.min.js', array(), false, false );
...
function page_load_progress_bar_css() {
	$output="<style> #page {
		opacity: 0;
		visibility: hidden;
...
add_action( 'wp_head', 'page_load_progress_bar_css' );
...
function mega_wpcf7_ajax_loader () {
	return get_template_directory_uri() .'/images/blank.gif';
}
...
function mega_posts( $atts, $content = null ) {
	...
	<div class="wpb_teaser_grid wpb_content_element wpb_grid-alternative wpb_teaser_grid_post <?php echo sanitize_html_class( $posts_columns ); ?> <?php echo sanitize_html_class( $pagination_align_class ); ?> <?php echo sanitize_html_class( $css_class ); ?> <?php echo sanitize_html_class( $post_column_class ); ?> clearfix">
					...
					<?php while ( $wp_query->have_posts() ) : $wp_query->the_post(); ?>
						...
						<?php } else { ?>
							...
							<div class="hentry-text-wrapper">
								...
								?>
								<div class="entry-content"><?php echo wpautop( $content ); ?></div><div class="more-link-wrapper"><?php echo $link; ?></div>
							</div>
							</li><!-- #post-<?php the_ID(); ?> -->
						<?php } ?>
					<?php endwhile; ?>
...
function mega_portfolio( $atts, $content = null ) {
	...
	<?php } // End if ($filter!='' && $filter!='0') ?>
		...
		<div class="mt-loader spinner3"><div class="bounce1"></div><div class="bounce2"></div><div class="bounce3"></div></div>
		<div id="portfolio" class="<?php echo sanitize_html_class( $portfolio_columns ); ?> <?php echo sanitize_html_class( $portfolio_title_position_class ); ?> <?php echo sanitize_html_class( $text_align_class ); ?> <?php echo sanitize_html_class( $portfolio_margin ); ?> <?php echo sanitize_html_class( $Animation ); ?> <?php echo sanitize_html_class( $Portfolio_50_width_enable ); ?> clearfix">
		<div class="grid-sizer"></div>
		<div class="gutter-sizer"></div>
			<?php if ( $wp_query->have_posts() ) : ?>
				...
				<?php while ( $wp_query->have_posts() ) : $wp_query->the_post(); ?>
					...
					<?php if ( $style == '2' ) { ?>
						...
						<?php } else if ( $style == '5' ) { ?>
							...
							<article id="post-<?php the_ID(); ?>" <?php post_class( 'clearfix portfolio-style-5' ); ?>>
								<div class="content-wrapper">
									<?php if ( $thumbnail ) { ?>
										<div class="post-thumbnail clearfix">
										<?php $portfolio_highlight_text_color = get_post_meta( get_the_ID(), 'portfolio_highlight_text_color', true ); ?>
...
vc_map( array(
   ...
   "params" => array(
		...
		array(
		  ...
		  "heading" => __("Columns count", "mega"),
		  "param_name" => "columns",
		  "value" => array(__("4 - Default", "mega") => 4, 3, 2),
		  "description" => __('Select columns count.', "mega")
		),
		array(
		  "type" => "textfield",
		...
		array(
		  "type" => "dropdown",
		  "heading" => __("Text Align", "mega"),
		  "param_name" => "text_align",
		  "value" => array( __("Center", "mega") => "center", __("Left", "mega") => "left" ),
		  "description" => __('Select portfolio text align.', "mega")
		),
		array(
		  "type" => "dropdown",
		...
		array(
		  ...
		  "param_name" => "style",
		  "value" => array(__("1 - Default", "mega") => 1, 2, 3, 4, 5),
	...
	$output .= '</div>';
		...
		$output .= '<div class="person-desc">';
			...
			$output .= '<ul class="clearfix">';
				...
				if ( ! empty( $facebook ) ) {
					$output .= '<li><a class="social facebook" href="' . esc_url( $facebook ) . '">
						<span class="social-icon"></span>
					</a></li>';
				}
					
				if ( ! empty( $twitter ) ) {
					$output .= '<li><a class="social twitter" href="' . esc_url( $twitter ) . '">
						<span class="social-icon"></span>
					</a></li>';
				}
				
				if ( ! empty( $google_plus ) ) {
					$output .= '<li><a class="social gplus" href="' . esc_url( $google_plus ) . '">
						<span class="social-icon"></span>
					</a></li>';
				}
					
				if ( ! empty( $linkedin ) ) {
					$output .= '<li><a class="social linkedin" href="' . esc_url( $linkedin ) . '">
						<span class="social-icon"></span>
					</a></li>';
				}
					
				if ( ! empty( $dribbble ) ) {
					$output .= '<li><a class="social dribbble" href="' . esc_url( $dribbble ) . '">
						<span class="social-icon"></span>
					</a></li>';
				}
				
				if ( ! empty( $pinterest ) ) {
					$output .= '<li><a class="social pinterest" href="' . esc_url( $pinterest ) . '">
						<span class="social-icon"></span>
					</a></li>';
				}
				
				if ( ! empty( $instagram ) ) {
					$output .= '<li><a class="social instagram" href="' . esc_url( $instagram ) . '">
						<span class="social-icon"></span>
					</a></li>';
				}
				
				if ( ! empty( $flickr ) ) {
					$output .= '<li><a class="social flickr" href="' . esc_url( $flickr ) . '">
						<span class="social-icon"></span>
					</a></li>';
				}
				
				if ( ! empty( $tumblr ) ) {
					$output .= '<li><a class="social tumblr" href="' . esc_url( $tumblr ) . '">
						<span class="social-icon"></span>
					</a></li>';
				}
...
function mega_vc_marketing_tour( $atts, $content = null ) {
	...
	if ( $style == '1' ) {
		$style_class = 'marketing-tour-1';
	} else if ( $style == '2' ) {
		$style_class = 'marketing-tour-2';
	} else {
		$style_class = 'marketing-tour-3';
	}
	
	$output = '';
	$output .= '<div class="marketing-tour-wrapper clearfix '. sanitize_html_class( $CSSAnimation ) .' '. sanitize_html_class( $style_class ) .' '. sanitize_html_class( $css_class ) .'">';
		$output .= '<span class="marketing-tour">';
			if ( ! empty( $icon_name ) ) {
				if ( $icon_color == NULL ) {
					$output .= '<div class="icon-'. sanitize_html_class( $icon_name ) .' custom-pack-icon"></div>';
				} else {
					$output .= '<div class="icon-'. sanitize_html_class( $icon_name ) .' custom-pack-icon" style="color: '. esc_attr( $icon_color ) .';"></div>';
				}
			} else {
				$output .= $img['thumbnail'];
			}
...
vc_map( array(
  ...
  "params" => array(
	...
	array(
      "type" => "dropdown",
      "heading" => __("Style", "mega"),
      "param_name" => "style",
	   "value" => array( 1, 2, 3 ),
      "description" => __("Select marketing tour style.", "mega")
    ),
	$add_css_animation,
	array(
      "type" => "textfield",
...
function mega_vc_single_portfolio_nav( $atts, $content = null ) {
	//extract( shortcode_atts( array(
		//'portfolio_page_url' => '',
		//'style' => '1',
		//'thumb_size' => 'full',
	...
	<nav id="nav-pagination-single">
		<?php if ( get_next_post() ) { ?>
			<div class="nav-previous"><?php next_post_link( '%link', __( '<i class="nav-pagination-single-left"></i> Prev', 'mega' ) ) ?></div>
		<?php } ?>
		
		<?php $portfolio_page = ot_get_option( 'portfolio_page' ); ?>
		<?php if ( ! empty( $portfolio_page ) ) { ?>
			<div class="nav-back"><a href="<?php echo get_permalink( $portfolio_page ); ?>"><?php echo __("Back to the Projects", "mega") ?></a></div>
		<?php } ?>
	
		<?php if ( get_previous_post() ) { ?>
			<div class="nav-next"><?php previous_post_link( '%link', __( 'Next <i class="nav-pagination-single-right"></i>', 'mega' ) ) ?></div>
		<?php } ?>
	</nav>
	...
	<?php //} else { ?>
		...
		<?php
			...
			//$post_thumbnail_next = $p_img_large = '';

			//$post_thumbnail_next = wpb_getImageBySize(array( 'post_id' => $next_post_id, 'thumb_size' => $thumb_size ));
			//$thumbnail_next = $post_thumbnail_next['thumbnail'];
			//$p_img_large = $post_thumbnail_next['p_img_large'];
		?>
		...
		<?php
			...
			//$post_thumbnail_previous = $p_img_large = '';

			//$post_thumbnail_previous = wpb_getImageBySize(array( 'post_id' => $previous_post_id, 'thumb_size' => $thumb_size ));
			//$thumbnail_previous = $post_thumbnail_previous['thumbnail'];
			//$p_img_large = $post_thumbnail_previous['p_img_large'];
		?>

		<?php //echo $thumbnail_next; ?>
		<?php //echo $thumbnail_previous; ?>
	
	<?php //} ?>
<?php
}
add_shortcode( 'vc_single_portfolio_nav', 'mega_vc_single_portfolio_nav' );

vc_map( array(
  "name" => __("Single Portfolio Nav", "mega"),
  "base" => "vc_single_portfolio_nav",
  'show_settings_on_create' => false,
  "category" => __('Content', 'mega'),
  //"params" => array(
	//array(
     // "type" => "textfield",
      //"heading" => __("Portfolio page URL", "mega"),
      //"param_name" => "portfolio_page_url",
      //"description" => __("Enter the portfolio page URL. Used for the 'Back to portfolio' link.", "mega")
   // ),
	//array(
		  //"type" => "dropdown",
		  //"heading" => __("Style", "mega"),
		  //"param_name" => "style",
		  //"value" => array( 1, 2 ),
		  //"description" => __('Select nav style.', "mega")
		//),
		//array(
		  //"type" => "textfield",
		  //"heading" => __("Thumbnail size", "mega"),
